/*! angular-detour - v0.4.0 - 2013-06-11
...
define('common',[], function() {
  function Common() {
    ...
    //adapted from http://jsperf.com/extjs-vs-lodash-merge (no copyright info found)
...
define('UrlMatcher',['common'], function(common) {
  ...
  UrlMatcher.prototype.addParameter = function(id, pattern, names, params) {
    if (!/^\w+$/.test(id)) {
  ...
  UrlMatcher.prototype.quoteRegExp = function(string) {
    return string.replace(/[\\\[\]\^$*+?.()|{}]/g, '\\$&');
  };

  UrlMatcher.prototype.define = function(pattern) {
    ...
    while ((m = placeholder.exec(pattern))) {
      ...
      compiled += this.quoteRegExp(segment) + '(' + regexp + ')';
      this.addParameter(id, pattern, names, params);
    ...
    if (i >= 0) {
      ...
      common.forEach(search.substring(1).split(/[&?]/), function(val) { this.addParameter(val, pattern, names, params);}, this);
    } else {
      this.sourcePath = pattern;
      this.sourceSearch = '';
    }

    compiled += this.quoteRegExp(segment) + '$';
    segments.push(segment);
  ...
  UrlMatcher.prototype.newInstance = function(pattern) {
  ...
  UrlMatcher.prototype.concat = function (pattern) {
    ...
    var inst = this.newInstance(this.sourcePath + pattern + this.sourceSearch);
    return inst;
  };
  ...
  UrlMatcher.prototype.getParameterValues = function(match, searchParams) {
//    if (searchParams) {
      ...
      for (i=0; i<nPath; i++) {
        values[params[i]] = decodeURIComponent(match[i+1]);
      }
      for (/**/; i<nTotal; i++) {
  ...
  UrlMatcher.prototype.exec = function (path, searchParams) {
    ...
    return this.getParameterValues(m, searchParams);
...
define('detourProvider',['common', 'UrlMatcher', 'StateBase', 'couchPotato', 'templateFactory', 'detourModule'], function(common, UrlMatcher, StateBase) {
  ...
  function $DetourProvider(
    ...
    State.prototype.resetUrlHandlers = function() {
      ...
      this.urlHandlers= [];
      this.urlHandlers.push(this._buildRule(what, handler));

      if (this.aliases) {
        common.forEach(this.aliases, function(value, key) {
          var handler = value.charAt(0) === '^'
            ? value.substring(1)
            : (that.parent.navigable || that.root).preparedUrl.concat(value).source;

          this.urlHandlers.push(this._buildRule(key, handler));
        }, this);
      }
    ...
    State.prototype.tryHandle = function($injector, $location) {
      ...
      if (this.urlHandlers) {
        for (var i = 0; i < this.urlHandlers.length && !handled; i++) {
          handled = this.urlHandlers[i]($injector, $location);
          if (handled) {
            break;
          }
        }
